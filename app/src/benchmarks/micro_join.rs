...
pub fn join_sec_0() -> Result<()> {
    ...
    let rdd0 = sc.parallelize::<(i32, (String, String)), _, _>(vec![], col0_enc, 4);
    let col1 = vec![
        (1, "A1".to_string()),
        (1, "A2".to_string()),
        (2, "B1".to_string()),
    ...
    let rdd1 = sc.parallelize::<(i32, String), _, _>(vec![], col1_enc, 4);
    let rdd2 = rdd1.join(rdd0.clone(), 4);
...
pub fn join_sec_2() -> Result<()> {
    ...
    let now = Instant::now();
    let rdd0 = sc.parallelize::<(i32, i32), _, _>(vec![], vec0_enc, 8);
    let rdd1 = sc.parallelize::<(i32, i32), _, _>(vec![], vec1_enc, 8);
