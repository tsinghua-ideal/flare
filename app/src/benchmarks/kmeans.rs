...
pub fn kmeans_sec_0() -> Result<()> {
    ...
    let k = 10;
    let converge_dist = OText::<f64>::new(10.0, None, None);
...
pub fn kmeans_unsec_0() -> Result<()> {
    ...
    // TODO: need to change dir
    let dir = PathBuf::from("/tmp/pt_km");
    let lines = sc.read_source(LocalFsReaderConfig::new(dir, false), deserializer, fe, fd);
    let data_rdd = lines.flat_map(Fn!(|lines: Vec<String>| {
        Box::new(lines.into_iter().map(|line| {
            parse_vector::<>(line)
        })) as Box<dyn Iterator<Item = _>>
