...
pub fn q1_sec() -> Result<()> {
    ...
    let res = table.secure_collect().unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
...
pub fn q2_sec() -> Result<()> {
    ...
    let res = table_final.secure_take(100).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.get_pt().len());
...
pub fn q3_sec() -> Result<()> {
    ...
    let res = table_final.secure_take(10).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.get_pt().len());
...
pub fn q4_sec() -> Result<()> {
    ...
    let res = table_final.secure_collect().unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.get_pt().len());
...
pub fn q6_sec() -> Result<()> {
    ...
    let res = table.secure_reduce(Fn!(|x: f32, y: f32| x + y)).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res = {:?}", res.get_pt());
...
pub fn q14_sec() -> Result<()> {
    ...
    let res = table_final
        .secure_reduce(Fn!(|x: (f32, f32), y: (f32, f32)| (x.0 + y.0, x.1 + y.1)))
        .unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res = {:?}", res.get_pt());
...
pub fn q17_sec() -> Result<()> {
    ...
    let res = table_final
        .secure_reduce(Fn!(|x: f32, y: f32| x + y))
        .unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res = {:?}", res.get_pt() / 7.0);
...
pub fn te1_unsec() -> Result<()> {
    ...
    let deserializer0 = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TSupplier>>(&file).unwrap()
    }));
    let deserializer1 = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TCustomer>>(&file).unwrap()
    }));
    ...
    let table0 = sc
        ...
        .flat_map(Fn!(
            |v: Vec<TSupplier>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .key_by(Fn!(|x: &TSupplier| x.3))
        .map(Fn!(|x: (i32, TSupplier)| (x.0, x.1 .0)));
    let table1 = sc
        ...
        .flat_map(Fn!(
            |v: Vec<TCustomer>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .key_by(Fn!(|x: &TCustomer| x.3))
        .map(Fn!(|x: (i32, TCustomer)| (x.0, x.1 .0)));
...
pub fn q1_unsec() -> Result<()> {
    let sc = Context::new()?;

    let deserializer = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TLineItem>>(&file).unwrap()
    }));

    let now = Instant::now();
    let dir = PathBuf::from("/opt/data/pt_tpch_1g_lineitem");
    let table = sc
        .read_source(
            LocalFsReaderConfig::new(dir).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer),
        ...
        .flat_map(Fn!(
            |v: Vec<TLineItem>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .filter(Fn!(|x: &TLineItem| {
            let mut date = Date::new(1998, 12, 1).unwrap();
            date.subtract_days(90);
...
pub fn q2_unsec() -> Result<()> {
    let sc = Context::new()?;

    let deserializer_part = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TPart>>(&file).unwrap()
    }));
    let deserializer_supplier = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TSupplier>>(&file).unwrap()
    }));
    let deserializer_partsupp = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TPartSupp>>(&file).unwrap()
    }));
    let deserializer_nation = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TNation>>(&file).unwrap()
    }));
    let deserializer_region = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TRegion>>(&file).unwrap()
    }));

    let now = Instant::now();
    let dir_part = PathBuf::from("/opt/data/pt_tpch_1g_part");
    let dir_supplier = PathBuf::from("/opt/data/pt_tpch_1g_supplier");
    let dir_partsupp = PathBuf::from("/opt/data/pt_tpch_1g_partsupp");
    let dir_nation = PathBuf::from("/opt/data/pt_tpch_1g_nation");
    let dir_region = PathBuf::from("/opt/data/pt_tpch_1g_region");

    let table_part = sc
        .read_source(
            LocalFsReaderConfig::new(dir_part).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_part),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TPart>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .filter(Fn!(|x: &TPart| x.4.ends_with("BRASS") && x.5 == 15));
    let table_supplier = sc
        .read_source(
            LocalFsReaderConfig::new(dir_supplier).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_supplier),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TSupplier>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
    ...
    let table_partsupp = sc
        .read_source(
            LocalFsReaderConfig::new(dir_partsupp).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_partsupp),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TPartSupp>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .map(Fn!(|x: TPartSupp| x));
    let table_nation = sc
        .read_source(
            LocalFsReaderConfig::new(dir_nation).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_nation),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TNation>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .map(Fn!(|x: TNation| x));
    let table_region = sc
        .read_source(
            LocalFsReaderConfig::new(dir_region).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_region),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TRegion>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .map(Fn!(|x: TRegion| x));
    ...
    let res = table_final.take(100).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.len());
...
pub fn q3_unsec() -> Result<()> {
    let sc = Context::new()?;

    let deserializer_customer = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TCustomer>>(&file).unwrap()
    }));
    let deserializer_orders = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TOrders>>(&file).unwrap()
    }));
    let deserializer_lineitem = Box::new(Fn!(|file: Vec<u8>| {
        bincode::deserialize::<Vec<TLineItem>>(&file).unwrap()
    }));

    let now = Instant::now();
    let dir_customer = PathBuf::from("/opt/data/pt_tpch_1g_customer");
    let dir_orders = PathBuf::from("/opt/data/pt_tpch_1g_orders");
    let dir_lineitem = PathBuf::from("/opt/data/pt_tpch_1g_lineitem");

    let table_customer = sc
        .read_source(
            LocalFsReaderConfig::new(dir_customer).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_customer),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TCustomer>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
    ...
    let table_orders = sc
        .read_source(
            LocalFsReaderConfig::new(dir_orders).num_partitions_per_executor(NUM_PARTS_LOCAL),
            Some(deserializer_orders),
            None,
        )
        .flat_map(Fn!(
            |v: Vec<TOrders>| Box::new(v.into_iter()) as Box<dyn Iterator<Item = _>>
        ))
        .filter(Fn!(|x: &TOrders| x.4 < Date::new(1995, 3, 15).unwrap()))
        .map(Fn!(|x: TOrders| {
            let (o_orderkey, o_custkey, _, _, o_orderdate, _, _, o_shippriority, _) = x;
    ...
    let res = table_final.take(10).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.len());
...
pub fn q4_unsec() -> Result<()> {
    ...
    let res = table_final.collect().unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res.len() = {:?}", res.len());
...
pub fn q6_unsec() -> Result<()> {
    ...
    let res = table.reduce(Fn!(|x: f32, y: f32| x + y)).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res = {:?}", res);
...
pub fn q14_unsec() -> Result<()> {
    ...
    let table_lineitem = sc
        ...
        .map(Fn!(|x: TLineItem| (x.0 .1, (x.0 .5, x.0 .6))));
    let table_final = table_part.join(table_lineitem, NUM_PARTS).map(Fn!(|(
        _,
...
pub fn q17_unsec() -> Result<()> {
    ...
    let res = table_final.reduce(Fn!(|x: f32, y: f32| x + y)).unwrap();
    let dur = now.elapsed().as_nanos() as f64 * 1e-9;
    println!("Total time {:?} s", dur);
    println!("res = {:?}", res.unwrap() / 7.0);

    Ok(())
}

pub fn q18_unsec() -> Result<()> {
    let sc = Context::new()?;
