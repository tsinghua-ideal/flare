...
pub fn map_sec_1() -> Result<()> {
    ...
    let fd_mp = Fn!(|ve: Vec<u8>| {
        let buf0 = ve;
        let pt0: Vec<Vec<i32>> = ser_decrypt::<>(buf0); 
        pt0
    });
...
pub fn map_unsec_1() -> Result<()> {
    ...
    let mut data = (0..10_000_000).collect::<Vec<_>>();
    let now = Instant::now();
    let rdd0 = sc.make_rdd(data, vec![], fe, fd, 1);
    let rdd1 = rdd0.map(Fn!(|i: i32| {
        let mut v = Vec::new();
        v.push(i);
        v
