...
pub fn pearson_sec_0() -> Result<()> {
    ...
    let dir0 = PathBuf::from("/opt/data/ct_pe_a_107");
    let dir1 = PathBuf::from("/opt/data/ct_pe_b_107");
    ...
    let mx = (*x.reduce(Fn!(|a, b| a+b), Fn!(|x| x), Fn!(|x| x)).unwrap())/
        x.count().unwrap() as f64;
    let my = (*y.reduce(Fn!(|a, b| a+b), Fn!(|x| x), Fn!(|x| x)).unwrap())/
    ...
    let (upper, lowerx, lowery) = *x.zip(y.into(), fe_zip, fd_zip)
        ...
        .reduce(Fn!(|a: (f64, f64, f64), b: (f64, f64, f64)| (a.0+b.0, a.1+b.1, a.2+b.2)), Fn!(|x| x), Fn!(|x| x)).unwrap();
