...
pub struct MapPartitions<T: Data, U: Data, F>
where
...
impl<T: Data, U: Data, F> Clone for MapPartitions<T, U, F>
where
    F: Fn(usize, Box<dyn Iterator<Item = T>>) -> Box<dyn Iterator<Item = U>> + Clone,
{
    fn clone(&self) -> Self {
...
impl<T: Data, U: Data, F> MapPartitions<T, U, F>
where
    F: Fn(usize, Box<dyn Iterator<Item = T>>) -> Box<dyn Iterator<Item = U>> + Clone,
{
...
impl<T: Data, U: Data, F> OpBase for MapPartitions<T, U, F>
where
    F: Fn(usize, Box<dyn Iterator<Item = T>>) -> Box<dyn Iterator<Item = U>> + Send + Sync + Clone + 'static,
{
...
impl<T: Data, U: Data, F> Op for MapPartitions<T, U, F>
where
    F: Fn(usize, Box<dyn Iterator<Item = T>>) -> Box<dyn Iterator<Item = U>> + Send + Sync + Clone + 'static,
{
    type Item = U;
    ...
    fn compute(&self, ser_data: &[u8], ser_data_idx: &[usize]) -> Box<dyn Iterator<Item = Self::Item>> {
        ...
        let f_result = self.f.clone()(index, self.prev.compute(ser_data, ser_data_idx));
        Box::new(f_result)
    }
