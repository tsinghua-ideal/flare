    ...
    C: Data,
{
    ...
    fn call_free_res_enc(&self, data: *mut u8, marks: *mut u8, is_enc: bool, dep_info: &DepInfo) {
        match dep_info.dep_type() {
            ...
            21 =>  {
                crate::ALLOCATOR.set_switch(true);
                let cnt_buckets = unsafe { Box::from_raw(data as *mut Vec<Vec<ItemE>>) };
                drop(cnt_buckets);
    ...
    fn compute_start(&self, mut call_seq: NextOpId, input: Input, dep_info: &DepInfo) -> (*mut u8, *mut u8) {
        match dep_info.dep_type() {
            ...
            21  => {
                ...
                let (alpha, beta) = stage_comm((a, cnt), part_group.0, part_group.1, part_group.2, part_id)
                    .into_iter()
                ...
                let last_bin_info = stage_comm((last_bin_num, last_bin_size), part_group.0, part_group.1, part_group.2, part_id);
                assert_eq!(last_bin_info[part_id], (last_bin_num, last_bin_size));
                ...
                crate::ALLOCATOR.set_switch(false);
                (to_ptr(buckets_enc), meta_data_ptr as *mut u8)
            },
            22 => {
